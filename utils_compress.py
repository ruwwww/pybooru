...
INPUT_DIR = r"C:\Coding3\pybooru\downloads\qewie"        # Folder containing your big images
OUTPUT_DIR = r"C:\Coding3\pybooru\downloads\qewie_resized"  # Folder to save samples
...
JPEG_QUALITY_MAX = 85          # Max quality (standard images)
JPEG_QUALITY_MIN = 70          # Min quality (very large/long images)
...
def process_image(file_info):
    ...
    try:
        ...
        if aspect_ratio <= ratio_threshold_low:
            q_value = JPEG_QUALITY_MAX
        elif aspect_ratio >= ratio_threshold_high:
            q_value = JPEG_QUALITY_MIN
        else:
            # Linear interpolation
            progress = (aspect_ratio - ratio_threshold_low) / (ratio_threshold_high - ratio_threshold_low)
            q_value = int(JPEG_QUALITY_MAX - (progress * (JPEG_QUALITY_MAX - JPEG_QUALITY_MIN)))

        # 4. Save with Aggressive JPEG Optimization
        # These are the specific flags Danbooru uses for quality/size balance.
        thumb.write_to_file(
            output_path,
            Q=q_value,
            optimize_coding=True,       # Compute optimal Huffman coding tables
            interlace=True,             # Progressive JPEG (loads blurry -> sharp)
            trellis_quant=True,         # Advanced quantization (smaller size, better quality)
            overshoot_deringing=True,   # Reduces "ringing" artifacts near sharp edges
            strip=True,                 # Remove metadata (EXIF) to save space
            background=[255, 255, 255]  # Flatten transparent PNGs to white background
        )
        return True
...
def main():
    ...
    for f in files:
        ...
        if os.path.getsize(in_path) < MIN_FILESIZE_KB * 1024:
            ...
            out_path = os.path.join(OUTPUT_DIR, f"{filename_no_ext}_sample.jpg")
